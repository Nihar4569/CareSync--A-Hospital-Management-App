...
import ulogo from './Images/dlogin.gif'
...
export default function () {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      setLoading(true);
      const { data } = await axios.get(`${server}/hosp/doctor/find/${email}`, { withCredentials: true });
  
      const fetchedPassword = data.password ? data.password.toString().trim() : '';
      const inputPassword = password.trim();
  
      if (fetchedPassword === inputPassword) {
        setDdata(data);
        await audit(`Doctor portal login by Dr ${data.dname} from ${data.hname}`)
        toast.success(`Welcome ${data.dname}`);
      } else {
        setDdata("");
            ...
            <form class="space-y-4">
              ...
              <div>
                ...
                <div class="relative flex items-center">
                  <input value={email} onChange={(e)=>setEmail(e.target.value)} type="email" required class="w-full text-sm text-gray-800 border border-gray-300 px-4 py-3 rounded-lg outline-blue-600" placeholder="Enter Your Email" />
              ...
              <div>
                ...
                <div class="relative flex items-center">
                  <input value={password} onChange={(e)=>setPassword(e.target.value)} type="password" required class="w-full text-sm text-gray-800 border border-gray-300 px-4 py-3 rounded-lg outline-blue-600" placeholder="Enter password" />
              ...
              <div class="!mt-8">
                <button onClick={submitHandler} type="button" class="w-full shadow-xl py-3 px-4 text-sm tracking-wide rounded-lg text-white bg-blue-600 hover:bg-blue-700 focus:outline-none">
                  Log in
                </button>
              </div>
